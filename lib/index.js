...
brushmove = function (graph) {
    ...
    dots.classed("selected", function(d) { 
        ...
        result = first <= position && position <= second; 
        return result;

    });

    first = Math.floor(s[0]);
    second = Math.floor(s[1]) + 1;

    // Please note that options.data[x] should always
    // have the x_position of x + 1
    // eg. options.data[0] has an x_position of 1
    yugene_value_first = options.data[first-1].yugene_value

    // check that we don't go over the number of actual values
    // in the data store
    if (options.data.length < second){
        second = options.data.length;
    }
    yugene_value_second = options.data[second-1].yugene_value

    //data = get_breakdown_of_filtered_data(yugene_value_first,yugene_value_second);
    $('#values').html(yugene_value_first+' : '+ yugene_value_second);
}

// Should this go outside in the stub like the tooltips?
// callback to get the filtered data back using ajax
get_breakdown_of_filtered_data = function(yugene_value_first,yugene_value_second){
    // note that start is the lower number
    data = '{"value_start": '+ yugene_value_second + ',"value_end":' + yugene_value_first+'}';
    url = 'http://dev.stemformatics.org/expressions/return_breakdown_of_yugene_filtered_data';
    var request = $.ajax({
        type: "POST",
        data: data,
        url: url
